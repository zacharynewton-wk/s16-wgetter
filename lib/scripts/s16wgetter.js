        ...
        let promise = new Promise((resolve, reject) => {
            ...
            lineReader.on('line', (line) => {
                ...
                handleMasterIDXLine(line, outputDir, paddedCikList, (prog, tot) => {
                    progress(master_idx_files[i], prog, tot);
                }).then(() => {
                    lineReader.resume();
                });
            });
...
async function handleMasterIDXLine(line, outputDir, cikList, progress) {
    ...
    if (!cikList.indexOf(cik)) return;

    let path = splitLine[4];
    let splitPath = path.split('/');
    let accessionNumber = splitPath[3].replace('.txt', '').trim();
    let url = `https://www.sec.gov/Archives/${splitPath[0]}/${splitPath[1]}/${splitPath[2]}/${accessionNumber.replace('-', '')}/edgar.xml`;
    // download to `${outputDir}/${accessionNumber}.xml`
    await downloadFile(url, `${outputDir}/${accessionNumber}.xml`);
}
...
async function downloadFile(url, outputPath, progress) {
    ...
    await new Promise((resolve, reject) => {
        let request = https.get(url).on('response', (response) => {
            ...
            response.on('data', (chunk) => {
                ...
                if (progress) progress(downloaded, len);
            }).on('end', () => {
                file.end();
                resolve();
            })
