...
const secUrl = 'https://www.sec.gov';
...
async function handleMasterIDXLine(line, outputDir, cikList, progress) {
    ...
    let dirUrl = `${secUrl}/Archives/${splitPath[0]}/${splitPath[1]}/${splitPath[2]}/${accessionNumber.replace(/-/g, '')}/`;
    // let url = `${dirUrl}*.xml`;

    request('https://www.sec.gov/Archives/edgar/data/1445305/000101363116000018', (err, res, html) => {
        const linkListRegex = /("\/Archives\/edgar\/data\/[^"]+\.xml")/g;
        let linkList = html.match(linkListRegex);
        for (let i = 0; i < linkList.length; i++) {
            // download to `${outputDir}/${accessionNumber}.xml`
            let fileUrl = linkList[i].replace(/"/g, '');
            let fileName = fileUrl.replace(/^.*[\\\/]/, '');
            downloadFile(`${secUrl}${fileUrl}`, `${outputDir}/${accessionNumber}.xml`, progress);
        }
    })
}
